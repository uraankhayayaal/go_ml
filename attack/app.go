...
func CreateNN() {
	...
	input := [][]float64{
		[]float64{500, 50.74, 0}, []float64{2000, 45.12, 0}, []float64{4000, 48.75, 0},
		[]float64{300, 47.88, 0}, []float64{900, 48.58, 0}, []float64{450, 46.08, 0},
		[]float64{1200, 44.76, 0}}
	// input := make([][]float64, 0)

	// for index := 0; index < 10; index++ {
	// 	input = append(input, []float64{float64(1 / (10 - index)), 1, float64(2 * index)})
	// }

	// Теперь создаём "цели" - те результаты, которые нужно получить
	target := [][]float64{
		[]float64{51, 0}, []float64{48, 0}, []float64{49, 0},
		[]float64{51, 0}, []float64{51.7, 0}, []float64{48, 0},
		[]float64{51, 0}}
